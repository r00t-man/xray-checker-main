...
JSON-объект с датой оплаты по каждому серверу. Показывается в карточке сервера как `Оплачено до: DD-MM-YYYY`.
...
Имена серверов сравниваются после нормализации пробелов, поэтому лишние двойные или «невидимые» пробелы в названии не мешают сопоставлению.
...
Дата по умолчанию для всех серверов, у которых нет явного значения в `WEB_PAID_UNTIL`.
